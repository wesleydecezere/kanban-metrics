...
export function handleIssuesLabeledEvent(event: IssuesLabeledEvent) {
  console.log(
    `Issue #${event.issue.number} has been labeled at ${event.issue.updated_at}`
  );
...
export function handleIssuesEditedEvent(event: IssuesEditedEvent) {
  ...
  changes.body &&
    console.log(
      `Body has changed from ${changes.body.from} to ${event.issue.body}`
    );
  changes.title &&
    console.log(
      `Title has changed from ${changes.title.from} to ${event.issue.title}`
    );
}
